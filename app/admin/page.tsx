...
import { Users, Building2, CreditCard, FileCheck } from "lucide-react";
...
async function quickUpdateApplicationStatus(
...
async function openApplicationDocument(filePath: string) {
  ...
  <div className="hidden border-l border-slate-200 pl-4 text-sm font-bold text-slate-500 md:block">
    Admin Control Centre
          ...
          <Button className="px-5 py-3">New Notice</Button>
        </div>
      </header>
  ...
  <div className="relative">
    ...
    <p className="mt-4 max-w-3xl text-white/75">
      Manage members, payments, business applications, documents, approvals,
      and cooperative operations from one control centre.
    </p>
  </div>
</div>
  ...
  {[
    ...
    },
    {
      ...
      value: applications.filter(
        (application) => application.application_status === "field_verification"
      ).length,
    },
    {
  ...
  ? applications.length
  : value === "payments"
  ? payments.filter((payment) => payment.payment_status === "pending").length
  ...
  {Array.isArray(payment.members)
    ? payment.members[0]?.full_name || "Unknown"
    : payment.members?.full_name || "Unknown"}
</td>

                        <td className="py-4 text-slate-600">
  {Array.isArray(payment.members)
    ? payment.members[0]?.email || "-"
  ...
  >
    {creatingMember ? "Creating..." : "Create Member"}
        ...
        <p className="mt-2 text-sm text-slate-600">
          View registered cooperative members, share balances, portfolio values,
          ...
          <tbody>
            {members.map((member) => (
              <tr key={member.id} className="border-b">
                ...
                <td className="py-4 font-bold">
                  FCFA {Number(member.declared_dividends || 0).toLocaleString()}
                </td>
...
<Card className="mt-8">
  ...
  <div className="mt-8 rounded-3xl border border-[#0D2D6E]/20 bg-white p-6">
    ...
    <div className="mt-5 grid gap-5 md:grid-cols-3">
      ...
      <div className="rounded-2xl bg-slate-50 p-4">
        <p className="text-xs font-bold uppercase text-slate-500">Requested</p>
        <p className="mt-1 font-black text-[#0D2D6E]">
          FCFA {Number(editingApplication.requested_amount || 0).toLocaleString()}
        </p>
      </div>
    ...
    <div className="mt-4 grid gap-3 md:grid-cols-3">
      {applicationDocuments.map((document) => (
        ...
        >
          ...
          <Button
  onClick={() => openApplicationDocument(document.file_path)}
  ...
  View Document
    ...
    <div className="mt-6 flex gap-3">
      ...
      >
        ...
        <p className="mt-2 text-sm text-slate-600">
          Review submitted business financing applications and prepare them for field verification.
        </p>
      </div>
        ...
        <table className="w-full min-w-[1100px] text-left text-sm">
          <thead>
            <tr className="border-b text-xs uppercase tracking-widest text-slate-500">
              ...
              <th className="py-4">Guarantor</th>
              <th className="py-4">Status</th>
              <th className="py-4">Date</th>
              <th className="py-4">Action</th>
            </tr>
          ...
          <tbody>
            {filteredApplications.map((application) => (
              <tr key={application.id} className="border-b">
                ...
                <td className="py-4 font-bold">
                  FCFA {Number(application.requested_amount || 0).toLocaleString()}
                </td>

                <td className="py-4 font-bold">
                  FCFA {Number(application.daily_revenue_estimate || 0).toLocaleString()}
                </td>
...
<div className="flex flex-col gap-2 min-w-[180px]">
  ...
  <button
    onClick={() => quickUpdateApplicationStatus(application.id, "under_review")}
  ...
  <button
    onClick={() =>
      quickUpdateApplicationStatus(application.id, "field_verification")
    }
    className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50"
  >
    Field
  ...
  <button
    onClick={() => quickUpdateApplicationStatus(application.id, "approved")}
    className="w-full rounded-xl bg-green-50 px-3 py-2 text-xs font-bold text-green-700 hover:bg-green-100"
  >
    Approve
  </button>

  <button
    onClick={() => quickUpdateApplicationStatus(application.id, "rejected")}
    className="w-full rounded-xl bg-red-50 px-3 py-2 text-xs font-bold text-red-700 hover:bg-red-100"
  >
    Reject
  </button>

  <button
    onClick={() => quickUpdateApplicationStatus(application.id, "disbursed")}
    className="w-full rounded-xl bg-[#0D2D6E] px-3 py-2 text-xs font-bold text-white hover:opacity-90"
  >
    Disburse
  </button>
</div>
